...
def test_image_layer():
    layer = ngs.ImageLayer(source='precomputed://gs://neuroglancer-fafb-data/fafb_v14/fafb_v14_clahe',
                           name='fafb_v14_clahe')
...
def test_segmentation_layer():
    layer = ngs.SegmentationLayer(source={"url": "precomputed://gs://fafb-ffn1-20200412/segmentation",
                                          "subsources": {
                                                         "default": True,
                                                         "bounds": True,
                                                         "mesh": True,
    ...
    # Generate second identical layer
    layer2 = ngs.SegmentationLayer(source={"url": "precomputed://gs://fafb-ffn1-20200412/segmentation",
                                           "subsources": {
                                                          "default": True,
                                                          "bounds": True,
                                                          "mesh": True,
    ...
    comb = layer | layer2
    assert len(comb.segments) == 2


def test_annotation_layer():
    layer = ngs.SegmentationLayer(source="precomputed://gs://neuroglancer-20191211_fafbv14_buhmann2019_li20190805",
...
def test_mesh_layer():
    layer = ngs.SegmentationLayer(source="vtk://https://storage.googleapis.com/neuroglancer-fafb-data/elmr-data/FAFB.surf.vtk.gz",
                                  shader="void main() {emitRGBA(vec4(1.0, 0.0, 0.0, 0.5));}",
...
def test_scene_parsing():
    ...
    scene = ngs.NeuroGlancerScene.from_string(TEST_URL)
    assert isinstance(scene.make_url(), str)
    assert isinstance(scene.__str__(), str)
    assert scene == copy.copy(scene)


def test_scene_combining():
    # Parse scene from string
    scene1 = ngs.NeuroGlancerScene.from_string(TEST_URL)
    scene2 = ngs.NeuroGlancerScene.from_string(TEST_URL)
